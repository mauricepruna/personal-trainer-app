...
import { createSessionAction, markSessionCompleteAction, deleteSessionAction } from "@/lib/actions/calendar";
...
export function CalendarView({ initialSessions, workouts }: CalendarViewProps) {
  ...
  async function handleSchedule() {
    ...
    try {
      ...
      setFormWorkoutId("");
      setFormNotes("");
      // Optimistically add to local state with a temp id
  ...
  async function toggleComplete(session: CalendarSession) {
    await markSessionCompleteAction(session.id);
    setSessions((prev) =>
      prev.map((s) =>
        s.id === session.id
          ? { ...s, completed_at: s.completed_at ? null : new Date().toISOString() }
          : s
      )
    );
  }
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
        <div className="mb-3 flex items-center justify-between">
          ...
          <Button size="sm" onClick={() => setShowForm(true)}>+ {t.calendar.schedule}</Button>
        </div>
        ...
        {selectedSessions.length === 0 && !showForm ? (
          ...
          <div className="space-y-2">
            {selectedSessions.map((s) => (
              <div key={s.id} className="flex items-center justify-between rounded-lg border border-gray-100 p-2 dark:border-gray-800">
                <div>
                  <span className="font-medium text-gray-900 dark:text-white">{s.workout_name ?? "Workout"}</span>
                ...
                <div className="flex items-center gap-1">
                  <button
                    ...
                    className={`rounded px-2 py-1 text-xs font-medium ${s.completed_at ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400" : "bg-gray-100 text-gray-600 hover:bg-blue-100 hover:text-blue-700 dark:bg-gray-800 dark:text-gray-400"}`}
                  >
